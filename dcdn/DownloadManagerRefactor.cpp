    ...
    while (true) {
        {
            ...
            mCv.wait_for(
                lk, reapInterval, [&] { return !mDlEventsHttp.empty() || !mDlEventsP2p.empty() || !mEvents.empty(); });
        }
...
// need hold std::lock_guard<std::mutex> lk(mTasksMutex);
void DownloadManager::transition_(TaskId id, TaskStatus to)
{
...
bool DownloadManager::canFinalize_(TaskId id)
{
    ...
    }
    {
        ...
        if (it != p2pStates_.end()) {
            hasP2pQueryInflight = std::any_of(
                it->second.begin(), it->second.end(), [](const auto& kv) { return kv.second.queryInFlight; });
        }
    }
...
void DownloadManager::applyFsm_(TaskId id, const FinalizeCheck&)
{
    ...
    // transit task status
    {
        ...
        auto& t = itT->second;
        if (t.Cancelled) {
            transition_(id, TaskStatus::Cancelled);
        } else {
            ...
            transition_(id, TaskStatus::Completed);
        }
    }
...
void DownloadManager::applyFsm_(const IOData& ev)
{
    ...
    if (ev.isProbe && ev.contentLenHint > 0) {
        ...
        if (needSplit) {
            // 注意：splitTask 内部会定位并取消正在运行的 probe 子任务
...
{
    {
        ...
        if (it == mTasks.end()) {
            ...
            if (it->second.Status == TaskStatus::Pending) {
                transition_(id, TaskStatus::Running);
            }
...
bool DownloadManager::p2pQueryPeersAsync_(TaskId id, size_t start, size_t end)
{
    ...
    auto succ = [this, id, start, end](nlohmann::json& res) { this->onP2PPeerQuerySuccess_(id, res, start, end); };
    auto fail = [this, id, start](int code) { this->onP2PPeerQueryFail_(id, code, start); };
...
void DownloadManager::onP2PPeerQueryFail_(TaskId id, int errCode, size_t start)
{
    ...
    if (it != mTaskOptions.end() && it->second.Strategy == DownloadStrategy::P2P_ONLY) {
...
{
    std::lock_guard<std::mutex> g(mSubMutex);
    auto id = mNextSubId++;

...
bool DownloadManager::PersistenceHelper::deleteTask(uint64_t taskId) { (void)taskId; return true; }
...
void DownloadManager::run()
{
    ...
    while (true) {
        // 等待：若无 FunctionCall 事件，可被下载器通知（mCv）唤醒
        {
            ...
            mCv.wait(lk, [&]{
                return !mDlEvents.empty() || !mEvents.empty();
            });
        }
        ...
        std::vector<std::shared_ptr<dcdn::util::DownloaderTask>> batch;
        {
            ...
            for (auto& kv : mDlEvents) batch.push_back(kv.second);
            mDlEvents.clear();
        }
        for (auto& t : batch) {
            try { processDownloaderEvent(t); }
            catch (...) {}
        }
        ...
        if (now - lastReap >= reapInterval) {
            ...
            std::vector<ActiveSubTask> snapshot;
            {
                ...
                for (auto& kv : mActiveByPtr) snapshot.push_back(kv.second);
            }

            for (auto& st : snapshot) {
                if (st.isProbe || st.length == 0) continue;
                ...
                if (now - st.lastTouched > staleTimeout) {
                    ...
                    if (paused2 || cancelled2) {
                        logWarn << "[watchdog] 任务处于暂停/取消，缺口已回填，不续排";
                        continue;
                    }
                    ...
                    bool hasNext = false;
                    {
                        ...
                        if (!q.empty()) { r = q.front(); q.pop_front(); hasNext = true; }
                    }
                    if (hasNext) {
                        std::string urlLocal;
                        FileDownloadOptions opts2;
                        {
                            ...
                            if (itT != mTasks.end()) urlLocal = itT->second.Url;
                            auto itOpt = mTaskOptions.find(st.parentTaskId);
                            if (itOpt != mTaskOptions.end()) opts2 = itOpt->second;
                        }
                        ...
                        try {
                            ...
                            if (sub) {
                                std::shared_ptr<std::fstream> f;
                                {
                                    ...
                                    if (itF != mParentFiles.end()) f = itF->second;
                                ...
                                logWarn << "[watchdog] 子任务卡死，回填续排 range: "
                                        << r.start << "-" << r.end;
                            } else {
                                logWarn << "[watchdog] CreateTask 失败，稍后重试 range: "
                                        << r.start << "-" << r.end;
                                std::lock_guard<std::mutex> l(mMtx);
            ...
            // 巡检后看能否 finalize
            {
                std::vector<uint64_t> toCheck;
                {
                    ...
                    for (auto& kv : mDlByTask) toCheck.push_back(kv.first);
                }
                for (auto id : toCheck) {
...
void DownloadManager::coreNotifyCallback(std::shared_ptr<dcdn::util::DownloaderTask> task, void* receiver)
{
    ...
    auto* self = static_cast<DownloadManager*>(receiver);
    {
        ...
        if (self->mCancelledRaw.count(task.get())) return; // 丢弃取消后的迟到回调
...
void DownloadManager::processDownloaderEvent(std::shared_ptr<dcdn::util::DownloaderTask> ev)
{
    ...
    // 任务取消后的迟到通知直接丢弃
    {
        ...
        if (mCancelledRaw.count(raw)) return;
    }
    ...
    // 暂停/取消保护：暂停状态直接不读、不续排（把事件丢弃）
    {
        std::lock_guard<std::mutex> lk(mTasksMutex);
        auto it = mTasks.find(active.parentTaskId);
        if (it != mTasks.end() && (it->second.Paused.load() || it->second.Cancelled.load())) {
            return;
        }
    }
...

        {
            ...
            if (it != mTasks.end() && it->second.TotalSize == 0) {
                if (auto ht = dynamic_cast<dcdn::util::HttpDownloaderTask*>(raw)) {
                    ...
                    if (itOpt != mTaskOptions.end()) opts = itOpt->second;
                        ...
                        if (opts.HasRange) {
                            ...
                            if (opts.RangeEnd != SIZE_MAX) {
                                totalLen = (opts.RangeEnd >= opts.RangeStart)
                                         ? (opts.RangeEnd - opts.RangeStart + 1) : 0;
                            } else {
    ...
    }
    {
        ...
        if (itF != mParentFiles.end()) file = itF->second;
    }
    ...
    while (buffer) {
        ...
        if (opts.HasRange && opts.WriteRangeToSeparateFile) {
            if (off >= opts.RangeStart) writeOff = off - opts.RangeStart;
            else writeOff = 0;
        }

        if (file && file->good()) {
            try { file->seekp(static_cast<std::streamoff>(writeOff), std::ios::beg); } catch (...) {}
        ...
        if (opts.StreamCb) {
            opts.StreamCb(reinterpret_cast<const char*>(buffer->Data()), len, off);
        }
    ...
    uint64_t got = 0;
    {
        std::lock_guard<std::mutex> l(mMtx);
        auto it = mActiveByPtr.find(raw);
        if (it != mActiveByPtr.end()) got = it->second.actualGot;
    }
    ...
    if ((isEnd || endByLength) && active.parentTaskId != 0) {
        if (shortRead) {
            ...
            logWarn << "子任务 " << active.index << " 短读, got=" << got
                    << " < need=" << active.length
                    << ", 回填缺口: [" << (active.offset+got) << ", "
                    << (active.offset + active.length - 1) << "]";
        } else {
            ...
            if (!active.isProbe && active.length > 0) {
                logInfo << "子任务 " << active.index << " 结束, start: " << active.offset
                        << ", end: " << (active.offset + active.length - 1)
                        << ", actually got: " << actualGot
                        << ", expect length: " << active.length
                        << (endByLength && !isEnd ? " (size-guard)" : "");
            } else {
...
// ============ 配置接口 ============
void DownloadManager::SetStrategy(DownloadStrategy strategy) { mStrategy = strategy; }
void DownloadManager::SetMaxConcurrentDownloads(size_t max)  { mMaxConcurrent = max; }
    ...
    const FileDownloadOptions& options)
{
    ...
    this->PostEvent(std::make_shared<ArgEvent<std::function<void()>>>(
        ...
        [this, prom, url, contentHash, options]() {
            DownloadTask task;
            task.Id = genTaskId();
            task.Url = url;
            ...
            if (!options.OutputPath.empty()) {
                auto fs = std::make_shared<std::fstream>(options.OutputPath, std::ios::in | std::ios::out | std::ios::binary);
                if (!fs->is_open()) {
            ...
            if (mStrategy == DownloadStrategy::HTTP_ONLY) {
                ...
                if (wantRange && endKnown) {
                    ...
                    if (length == 0) {
                        ...
                        if (itT != mTasks.end()) itT->second.Status = TaskStatus::Failed;
...

                {
                    ...
                    if (itF != mParentFiles.end()) a.file = itF->second;
...

                {
                    std::lock_guard<std::mutex> lk(mTasksMutex);
                    auto itT = mTasks.find(task.Id);
                    if (itT != mTasks.end()) itT->second.Status = TaskStatus::Running;
                }
                prom->set_value(task.Id);
                return;
            }
            ...
            // 其他策略暂未实现
            {
                std::lock_guard<std::mutex> lk(mTasksMutex);
                auto itT = mTasks.find(task.Id);
                if (itT != mTasks.end()) itT->second.Status = TaskStatus::Pending;
            }
            prom->set_value(task.Id);
        }
    ));
        ...
        [this, prom, taskId]() {
            {
                ...
                if (it == mTasks.end()) { prom->set_value(false); return; }
            ...
            // 取消所有子任务 & 标记 raw 已取消
            {
                ...
                for (auto it2 = mDlEvents.begin(); it2 != mDlEvents.end();) {
                    if (mCancelledRaw.count(it2->first)) it2 = mDlEvents.erase(it2);
                    else ++it2;
                }
                // 删除输出文件（如果指定了文件路径）
                FileDownloadOptions opts;
    ...
    this->PostEvent(std::make_shared<ArgEvent<std::function<void()>>>(
        ...
        [this, prom, taskId]() {
            ...
            }
            {
                ...
                if (itVec != mDlByTask.end()) {
                    for (auto& sp : itVec->second) {
                        if (sp) mHttpDownloader->PauseTask(sp);
                    }
                }
            }
            prom->set_value(true);
            ...
            }
            {
                ...
                if (itVec != mDlByTask.end()) {
                    for (auto& sp : itVec->second) {
                        ...
                        if (sp) mHttpDownloader->ResumeTask(sp);
                    }
                }
            }
            prom->set_value(true);
...
DownloadTask DownloadManager::GetTaskStatus(uint64_t taskId) const
{
    ...
    auto it = mTasks.find(taskId);
    if (it != mTasks.end()) return it->second;
    return {};
}
...
std::vector<DownloadTask> DownloadManager::GetAllTasks() const
{
    ...
    for (auto& kv : mTasks) out.push_back(kv.second);
    return out;
}
...
double DownloadManager::GetOverallSpeed() const
{
    std::lock_guard<std::mutex> l(mTasksMutex);
    double sum = 0.0;
    for (auto& kv : mTasks) sum += kv.second.Speed;
    return sum;
}
...
std::vector<std::pair<size_t, size_t>> DownloadManager::GetAvailableRanges(uint64_t taskId) const
{
    std::lock_guard<std::mutex> l(mTasksMutex);
    auto it = mTasks.find(taskId);
    if (it != mTasks.end()) return it->second.CompletedRanges;
    return {};
}

void DownloadManager::SetHttpBandwidthRatio(float ratio) { mHttpBandwidthRatio = ratio; }
void DownloadManager::SetP2pBandwidthRatio(float ratio)  { mP2pBandwidthRatio = ratio; }
...
void DownloadManager::updateTaskProgress(uint64_t taskId, size_t downloaded)
{
...

    {
        ...
        if (it->second.TotalSize > 0 && it->second.Downloaded >= it->second.TotalSize) {
            ...
            reachedEnd = true;
            // TODO: 找到为什么100%到这里才能complete
            it->second.Status = TaskStatus::Completed;
            logWarn << "Task " << taskId << " downloaded more than expected: "
...
// ============ 预留策略入口（未实现） ============
void DownloadManager::startHttpDownload(const uint64_t& taskId) { (void)taskId; }
void DownloadManager::startP2pDownload(const uint64_t& taskId)  { (void)taskId; }
void DownloadManager::startHybridDownload(const uint64_t& taskId){ (void)taskId; }
...
void DownloadManager::splitTask(uint64_t taskId, size_t totalSize, size_t baseOffset)
{
    ...
    if (already > 0) {
        ...
        if (probeEnd >= absBegin) rangeStart = probeEnd + 1;
    ...
    std::shared_ptr<std::fstream> parentFile;
    {
        ...
        if (itF != mParentFiles.end()) parentFile = itF->second;
    ...
    if (paused || cancelled || canLaunch == 0) {
        ...
        for (auto& r : ranges) q.push_back(r);
        maybeFinalizeTask(taskId);
        return;
    }
...
bool DownloadManager::maybeFinalizeTask(uint64_t taskId)
{
    ...
    if (hasRunning || hasPending) return false;

    {
        std::lock_guard<std::mutex> lk(mTasksMutex);
        auto itT = mTasks.find(taskId);
        if (itT == mTasks.end()) return false;

...
public:
    ...
    FileManager(MainManager *man, const FileManagerOption &opt) : BaseManager(man),
                                                                  mOpt(opt)
    {
    }
    ~FileManager();
...
public:
    std::string FilePath(const FileDescriptor &file)
    {
        if (std::holds_alternative<BlockInfo>(file))
        {
            ...
            return "blk_" + block.file_hash + "_" + std::to_string(block.block_start) + "_" + std::to_string(block.block_end);
        }
...
private:
    ...
    std::mutex mLRUMutex;
    std::unordered_map<uint64_t, LRUNode> mLRUCache;  // file_id -> LRUNode，统一管理访问记录
